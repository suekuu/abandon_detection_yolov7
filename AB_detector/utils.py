...
def count_overtime_trackid(frame_id, time_counter, track_id_coordinates, frame_counter, move_class, time_thresthold,
                           lost_threshold, skip_frames):
    '''
    ...
    for (track_id, time_) in list(time_counter.items()):
        if (time_ - 1) * skip_frames + 1 > time_thresthold:
            ...
            if move_class[track_id]['move_num'] >= move_class[track_id]['stop_num']:
                move_stop_classes.append('moving-' + str(move_class[track_id]['stop_num']) + '-' + str(
                    move_class[track_id]['move_num']) + '-' + str(track_id))
            else:
                move_stop_classes.append('stopping-' + str(move_class[track_id]['stop_num']) + '-' + str(
                    move_class[track_id]['move_num']) + '-' + str(track_id))
