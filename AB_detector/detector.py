            ...
            if dataset.frame % configs.skip_frames != 0:
                ...
                if result_sort is None:  # 跟踪为空，则跳过
                    ...
                    for subs_abandon in detected_abandon:
                        ...
                        # 判断遗留时间 从时间 转成 帧数量
                        return_abandon_coordinates, move_stop_classes = count_overtime_trackid(dataset.frame,
                                                                                               time_counter,
                                                                                               track_id_coordinates,
                                                                                               frame_counter,
                                                                                               abandon_status,
                                                                                               configs.abandon_stop_time,
                                                                                               configs.lost_threshold,
                                                                                               configs.skip_frames)
                        ###
                        print(return_abandon_coordinates, move_stop_classes)
                        coordinates = subs_abandon['axis']
                        track_id = subs_abandon['track_id']
                        if len(list(subs_abandon['attribute'])) == 1 and list(subs_abandon['attribute'])[0] in [i for
                                                                                                                ind, i
                                                                                                                in
                                                                                                                enumerate(
                                                                                                                        names)
                                                                                                                if
                                                                                                                ind in configs.abandon_list]:
